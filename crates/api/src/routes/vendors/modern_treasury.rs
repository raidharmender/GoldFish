...
async fn webhook(req: HttpRequest, body: Bytes) -> impl Responder {
  enqueue(&req, "modern_treasury", &body).await;
  match serde_json::from_slice::<serde_json::Value>(&body) {
    Ok(v) => HttpResponse::Ok().json(serde_json::json!({"vendor":"modern_treasury","payload":v})),
...
async fn enqueue(req: &HttpRequest, scope: &str, body: &[u8]) {
  ...
  if let Ok(true) = jobs::claim_idempotency(&pool, scope, &key).await {
    let _ = jobs::enqueue(&pool, "webhook.modern_treasury", serde_json::json!({"body_sha256": key}), Utc::now()).await;
  }
}
